...
This guide explains how to configure µStreamer to provide a H.264 video stream via WebRTC and consume that stream within a web application.
...
First, create a symbolic link to the Janus header files from `/usr/include` to the Janus install directory. By default, Janus installs to `/opt/janus`.

```sh
ln -s /opt/janus/include/janus /usr/include/janus
```

Janus uses nonstandard `#include` paths that break external compilation. To fix this, modify the `#include` directive in `janus/plugins/plugin.h` file to prepend a `../` to the included file name (`#include "refcount.h"` → `#include "../refcount.h"`).

```sh
sed \
  --in-place \
  --expression 's|^#include "refcount.h"$|#include "../refcount.h"|g' \
  /usr/include/janus/plugins
```
...
The `WITH_JANUS` option compiles the µStreamer Janus plugin to `janus/libjanus_ustreamer.so`. Move this file to the plugin directory of your Janus installation.
...
In order to share the video stream with the µStreamer Janus plugin, µStreamer must run with the following command-line flags:
